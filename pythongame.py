...
def main():
    ...
    if choice == "2":
        ...
        inventory, money, monsters = load_game(filename)
    ...
    while hp > 0:
        ...
        if choice in ['1', 'fight']:
            ...
            if item == "1":
                ...
                if qty:
                    inventory.append({"name": "Pokeball", "type": "misc"})
                    print("Pokeball added to inventory.")
            elif item == "2":
                qty, money = purchase_item(50.00, money)
                if qty:
                    inventory.append({"name": "Greatball", "type": "misc"})
                    print("Greatball added to inventory.")
            elif item == "3":
                qty, money = purchase_item(100.00, money)
                if qty:
                    inventory.append({"name": "Quick Claw Knife", "type": "weapon", "maxDurability": 15, "currentDurability": 15})
                    print("Quick Claw Knife added to inventory.")
            elif item == "4":
                qty, money = purchase_item(150.00, money)
                if qty:
                    inventory.append({"name": "Mewtwo Saber", "type": "consumable", "effect": "auto-defeat"})
                    print("Mewtwo Saber added to inventory.")
            elif item == "5":
                qty, money = purchase_item(15.00, money)
                if qty:
                    inventory.append({"name": "RazzBerry", "type": "consumable"})
                    print("RazzBerry added to inventory.")
            elif item == "6":
                qty, money = purchase_item(25.00, money)
                if qty:
                    inventory.append({"name": "NanabBerry", "type": "consumable"})
                    print("NanabBerry added to inventory.")
            elif item == "7":
                qty, money = purchase_item(20.00, money)
                if qty:
                    inventory.append({"name": "Antidote", "type": "consumable"})
                    print("Antidote added to inventory.")
            elif item == "8":
                qty, money = purchase_item(25.00, money)
                if qty:
                    inventory.append({"name": "Awakening", "type": "consumable"})
                    print("Awakening added to inventory.")
            else:
                print("No purchase made.")

        elif choice in ['5', 'save', 'save & quit']:
            filename = input("Enter save file name: ").strip()
            save_game(filename, inventory, money, monsters)
            ...
            encounter, monsters = run_map(position, monsters)
            
            if encounter and encounter["type"] == "town":
                print("You rest and heal up in the town.")
                hp = min(hp + 20, 500)
            elif encounter and encounter["type"] == "monster":
                print(f"You encountered a wild {encounter['monster'].name}!")
                hp, money = fight(hp, money, encounter['monster'], inventory)
